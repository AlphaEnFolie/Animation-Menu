...
---Commence l'animation en fonction du type de données
...
---Renvoie le style de marche actuel enregistré 
---@return string
    ...
    if data then
        ...
        if savedWalk then 
            ...
            if result.passed then
                Play.Notification('info', 'Remettre en place l ancien style de marche.')
            end
        end
    end
    cb({})
...
RegisterCommand(cfg.commandNameEmote, function(_, args)
    ...
    Play.Notification('info', 'Aucun nom d émote défini...')
end)
...
---Événement de mise à jour de cfg à partir d'une autre ressource
---@param _cfg table
---@param result any
---@return any
AddEventHandler('anims:updateCfg', function(_cfg, result)
    if GetCurrentResourceName() == GetInvokingResource() then
        CancelEvent()
        return print('Impossible d utiliser cet événement à partir de la même ressource!')
    end
    if type(_cfg) ~= "table" then
        print(GetInvokingResource() .. ' j ai essayé de mettre à jour anims cfg mais ce n était pas une table')
        CancelEvent()
        return
    end
    local oldCfg = cfg
    ...
    print(GetInvokingResource() .. ' animations cfg mises à jour!')
    if result then

---Maintient la lecture de l'animation
---@class Play
Play = {}

----AlphaEnFolieee----

---Vérifie le sexe du ped
---@return string
...
---Avertir une personne avec une notification par défaut
...
---Change l'animation de marche d'un ped
---@param walks table
---@param p table Promise
...
---Crée un ou plusieurs accessoires
---@param props table
...
---Envoyer l'événement au serveur pour l'animation
---@param shared table
---@param p table
Play.Shared = function(shared, p)
    if shared then
        ...
        if closePed then
            ...
            Play.Notification('info', 'Demande envoyée à ' .. GetPlayerName(targetId))
...
Play.Notification = function(type, message)
    if cfg.useTnotify then
        exports['t-notify']:Alert({
            style  =  type or 'info',
            message  =  message or 'Quelque chose s est mal passé...'
        })
    else
        notify(message)
    end
...
--- l'animation partagée si elle est acceptée
---@param shared table
...
---Charge la confirmation partagée pour la cible
---@param target number
---@param shared table
...
---Juste une fonction de notification
---serveur à envoyer à la cible
---@param type string
---@param message string
